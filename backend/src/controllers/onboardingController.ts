...
class OnboardingController {
  ...
  async register(req: Request, res: Response) {
    try {
      ...
      // Include email for display, but exclude password for security
      const dynamicData: Record<string, any> = {};
      
      for (const field of form.fields || []) {
        // Skip password field (security), but include email field (for display)
        if (!field.is_password_field) {
      ...
      await db.run(
        `INSERT INTO ${targetTable} (${columns.map(c => `"${c}"`).join(', ')}) VALUES (${placeholders})`,
        values
      );
      ...
      logger.info(`User also inserted into dynamic_users for QR compatibility`);
      ...
      // Generate QR code using QRService - use dynamicUserId (the ID in dynamic_users table)
      const qrResult = await QRService.generateSecureQR(dynamicUserId, tableExists?.id || dynamicUserId);
  ...
  async registerWithForm(req: Request, res: Response) {
    try {
      ...
      // Generate JWT token for auto-login (same as main register function)
      const token = jwt.sign(
        {
          id: coreUser.id,
          email: coreUser.email,
