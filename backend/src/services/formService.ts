...
class FormService {
  async getActiveForm(): Promise<FormDefinition | null> {
    try {
      const form = await db.get(
        `SELECT id, name as form_name, description, target_table, is_active, created_at, updated_at FROM form_definitions WHERE is_active = 1 LIMIT 1`
      );
  ...
  async getAllForms(): Promise<FormDefinition[]> {
    try {
      const forms = await db.all(
        `SELECT id, name as form_name, description, target_table, is_active, created_at, updated_at FROM form_definitions ORDER BY created_at DESC`
      );
  ...
  async getFormById(formId: string): Promise<FormDefinition | null> {
    try {
      const form = await db.get(
        `SELECT id, name as form_name, description, target_table, is_active, created_at, updated_at FROM form_definitions WHERE id = ?`,
        [formId]
      );
